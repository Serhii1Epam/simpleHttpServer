...
func TestCheckPasswordHash(t *testing.T) {
	...
	testHashData := hasher.NewHasher("TestPass")
	if !testHashData.CheckPasswordHash(want) {
...
func TestHashPassword(t *testing.T) {
	...
	testHashData := hasher.NewHasher("TestPass1")
	if got, err := testHashData.HashPassword(); got != want {
		fmt.Printf("%x", got)
